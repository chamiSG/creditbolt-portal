...
const FormIdentity = (props: FormProps) => {
  ...
  const inputElementsByCondition = (field: FormFieldProps) => {
    ...
    //   return <Input as={Autocomplete} type={type} apiKey={process.env.GOOGLE_MAPS_API_KEY} {...register(name, rules)} />
    // } 
    ...
    return (
      <GridItem colSpan={{ base: 1, md: colSpan }} key={index}>
        <FormControl isInvalid={!!errors[name]}>
          <FormLabel htmlFor={name}>{label} {optional ? '(Optional)' : ''}</FormLabel>
          <InputGroup>
            {icon &&
              <InputLeftElement pointerEvents='none'>
                <Icon as={icon} color={'lime.500'} />
              </InputLeftElement>
            }
            {addon &&
